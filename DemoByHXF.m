...
%input_sparse_path = '/home/yangfei/Workspace/HFM/LidarSeg/test_images_sparse';
input_sparse_path = '/home/yangfei/Desktop/test_mask';
output_path = '/home/yangfei/Desktop/Markov';
filelist=dir(input_color_path);
%% Read data
for frameID=3:length(filelist)
close all;
fprintf([filelist(frameID).name,'\n']);
split1 = regexp(filelist(frameID).name, '_','split'); 
DepthFP = sprintf('%s/%s',input_sparse_path,[cell2mat(split1(1)),'_road_',cell2mat(split1(2))]);    % spare image
...
%% Trim data if needed
% ColorSection = imresize(Color,[375,1242]);
...
%{
ColorHis = zeros(255,3);
for i=1:Height
    for j=1:Width
        for k=1:3
            ColorHis(ColorSection(i,j,k)+1,k) = ColorHis(ColorSection(i,j,k)+1,k) + 1;
        end
    end
end
...
for i=1:Height
    for j=1:Width
        ...
        for k=1:3
            for colori=1:255
                Temp1 = (Color(i,j,k) - colori);
                Temp2 = double(Temp1) * double(Temp1);
                sum = sum + Temp2  * ColorHis(colori,k);
            end
        end
        Sal(i,j) = double(sqrt(double(sum)) / (Height*Width));
    end
    i
end
 
imwrite(uint8(DepthSection),['./data/KITTI/vs.png'],'png')
%}
%% Set Parameters
...
% WFP = sprintf('/media/yangfei/Repository/KITTI/odometry/dataset/sequences/00/depth_2/%06d.png', frameID);
WFP = sprintf('%s/%s',output_path,filelist(frameID).name);
% imwrite(Result/65535.0,WFP,'png','bitdepth',16);
