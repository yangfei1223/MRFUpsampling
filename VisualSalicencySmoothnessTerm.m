function output = VisualSalicencySmoothnessTerm(color,sigma,sal)
...
SalUp = [sal;zeros(1,width)]; 
SalUp = SalUp(2:height+1,:);         % 整个矩阵上移动一行，最后一行用0填充
SalLeft = [sal,zeros(height,1)]; 
SalLeft = SalLeft(:,2:width+1);      % 整个矩阵左移动一列，最后一列用0填充
CompareSal{1}=SalUp;
CompareSal{2}=SalLeft;
...
for i=1:2 % 针对四幅相减图像进行操作
    ...
    STemp1 = sal(rowRange{i},colRange{i}) - CompareSal{i}(rowRange{i},colRange{i});
    STemp2 = STemp1(:,:).^2;
